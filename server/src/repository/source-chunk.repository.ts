...
export type sourceChunkRecord = Prisma.SourceChunkGetPayload<{ select: typeof sourceChunkSelect }>;
...
export async function deleteSourceChunksBySourceId(sourceId: string) {
    return await prisma.sourceChunk.deleteMany({
        where: { sourceId }
    });
}
...
export async function createSourceChunks(chunks: createSourceChunkData[]) {
    ...
    return prisma.$transaction(
        chunks.map((chunk) => prisma.sourceChunk.create({
            data: {
...
export async function findChunksBySourceId(sourceId: string) {
